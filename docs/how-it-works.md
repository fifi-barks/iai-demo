...
5. **Three gates.** A plan (what changes), a security scan (Checkov + Trivy), and a cost estimate (Infracost) all run automatically. The security gate catches a security group left open to the internet and restricts it; the cost gate prices what was actually generated.
...
| Reasoning | **Groq** running **Llama 3.3 70B** | The intent parse was the slowest beat on a local model (~60–90s on Ollama/phi). Groq's LPU returns the same result sub-second on a free tier with no credit card. Because the endpoint is OpenAI-compatible, switching to Cerebras, OpenAI, or a local Ollama model is a one-line env change. |
